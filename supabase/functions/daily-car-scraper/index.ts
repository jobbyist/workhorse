...
const SITES = ['webuycars', 'carsza', 'autotrader'] as const;
...
async function scrapeFromSite(site: typeof SITES[number], firecrawlKey: string, limitPerSite: number) {
  ...
  try {
    ...
    const listings = scrapeData.data?.json?.listings || [];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Scrape from all sites in parallel
